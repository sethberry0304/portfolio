...
async function fetchFromWBI(uid: string): Promise<Work[]> {
  ...
  return raw.map((v) => {
    ...
    return {
      ...
      extra: { publishedAt: new Date(created * 1000).toISOString() },
    };
  });
}
...
async function fetchFromRSS(uid: string): Promise<Work[]> {
  ...
  for (const item of items) {
    ...
    list.push({
      ...
      href: `https://www.bilibili.com/video/${bvid}`,
      extra: { publishedAt: new Date(ts * 1000).toISOString() },
    });
  }
...
function writeToWorks(videos: Work[]) {
  ...
  const out = `export const works = ${JSON.stringify(sorted, null, 2)} as const;\n`;
  const dataPath = path.resolve(process.cwd(), 'data/works.ts');
  fs.mkdirSync(path.dirname(dataPath), { recursive: true });
  fs.writeFileSync(dataPath, out, 'utf8');
  console.log(`\n📊 Summary: total written = ${sorted.length}`);
  console.log('➡ data/works.ts 已生成/更新\n');
}
